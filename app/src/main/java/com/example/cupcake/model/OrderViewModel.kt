...
class OrderViewModel : ViewModel() {
    ...
    //Tipping or not
    private val _tip = MutableLiveData<Double>()
    ...
    /**
     * Set the tip amount for this order
     *
     * @param tipAmount is the tip amount as a double
     */
    fun setTip(tipAmount: Double){
        _tip.value = tipAmount
        updatePrice()
    }
    ...
    private fun updatePrice() {
        ...
        if(tip.value != 0.0){
            calculatedPrice += ((calculatedPrice * tip.value!!) / 100)
        }
        _price.value = calculatedPrice
    }
    /**
